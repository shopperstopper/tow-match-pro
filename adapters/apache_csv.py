...
def normalize_category(title, supplied)->str:
    t=str(title or '').lower(); s=str(supplied or '').lower()
...
def infer_major_type(title, category):
    ...
    if category=='Truck Camper':
        if any(x in t for x in ('pop-up','pop up','cirrus 620','cirrus 820','scout ')):
            return 'Pop-Up'
        return 'Hard-Side'
    if 'bunk' in t or re.search(r'\b(?:bh|bhs|bhw|bunk)\b', t):
        return 'Bunkhouse'
...
def load_apache_scraper_csv(path: str|Path):
    ...
    for _,r in df.iterrows():
        ...
        records.append({
            'display_title':title,
            'rv_category':normalize_category(title,r.get('Category')),
            'major_type':infer_major_type(title,normalize_category(title,r.get('Category'))),
